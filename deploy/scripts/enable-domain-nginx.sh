...
#
# Prefer Backend/infra/scripts/07-enable-domain-nginx.sh when working from Backend/.
set -euo pipefail
...
if [[ -z "${DOMAIN}" ]]; then
  ...
  echo "  bash deploy/scripts/enable-domain-nginx.sh"
  exit 1
fi
...
if [[ ! -f /var/www/admin/index.html ]]; then
  echo "WARNING: /var/www/admin/index.html missing — app.${DOMAIN} will 404 until dist/ is uploaded"
fi
...
echo "Stop before Day 9 certbot."
